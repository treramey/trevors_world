...
export function LatestThreeBlogPosts() {
	...
	return (
		<section className="grid-parent">
			<p className="text-2xl sm:text-xl grid-child-left font-mondwest">
				garden
			</p>
...
export function ListAllBlogPosts() {
	...
	const years = Array.from(
		new Set(
			posts.map((post) => new Date(post.metadata.publishedAt).getFullYear()),
		),
	);
			...
			<section key={year} className="grid-parent">
				...
				<div className="grid-child-center">
					{posts
						.filter(
							(post) =>
								new Date(post.metadata.publishedAt).getFullYear() === year,
						)
						.map((post) => (
							<BlogPost key={post.slug} post={post} />
						))}
